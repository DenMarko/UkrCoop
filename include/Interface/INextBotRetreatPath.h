...
/*Шлях відступу*/
class RetreatPath : public IPathFollower
{
public:
	RetreatPath( void );
	virtual ~RetreatPath() { }

	void Update( INextBot *bot, IBaseEntity *threat );	// update path away from threat and move bot along path
...
private:
	void RefreshPath( INextBot *bot, IBaseEntity *threat );

	CountdownTimers m_throttleTimer;						// require a minimum time between re-paths
	CHandle<IBaseEntity> m_pathThreat;								// the threat of our existing path
 ...
 */
inline void RetreatPath::Update( INextBot *bot, IBaseEntity *threat )
{
...
{
public:
	RetreatPathBuilder( INextBot *me, IBaseEntity *threat, float retreatRange = 500.0f )
	{
		m_me = me;
	...
	INavArea *ComputePath( void )
	{
		...
		INavArea *startArea = (INavArea*)reinterpret_cast<IBaseCombatCharacter*>(m_me->GetEntity())->GetLastKnownArea();
		...
		INavArea *retreatFromArea = pMesh->GetNearestNavArea( m_threat->GetAbsOrigin() );
		...
		int teamID = reinterpret_cast<IBaseCombatCharacter*>(m_me->GetEntity())->GetTeamNumber();
		...
		while( !INavArea::IsOpenListEmpty() )
		{
			...
			{
				INavArea *newArea = m_adjAreaVector[ i ].area;
				...
				if ( ( newArea->GetCenter() - reinterpret_cast<IBaseCombatCharacter*>(m_me->GetEntity())->GetAbsOrigin() ).IsLengthGreaterThan( m_retreatRange ) )
					continue;
	...
	void CollectAdjacentAreas( INavArea *area )
	{
		...
		{
			INavLadder *ladder = adjUpLadder[ it ].ladder;
		...
		{
			INavLadder *ladder = adjDownLadder[ it ].ladder;
	 ...
	 */
	float Cost( INavArea *area, INavArea *fromArea, const INavLadder *ladder )
	{
		// check if we can use this area
		if ( !m_mover->IsAreaTraversable( (CNavArea*)area ) )
		{
			return -1.0f;
		}
		...
		if ( fromArea == NULL )
		{
			...
			else
			{
				...
				float rangeToThreat = ( m_threat->GetAbsOrigin() - reinterpret_cast<IBaseCombatCharacter*>(m_me->GetEntity())->GetAbsOrigin() ).Length();
 ...
 */
inline void RetreatPath::RefreshPath( INextBot *bot, IBaseEntity *threat )
{
	...
	if ( !IsValid() || ( threat->GetAbsOrigin() - m_pathThreatPos ).IsLengthGreaterThan( tolerance ) )
	{
		...
		INavArea *goalArea = retreat.ComputePath();
