...
protected:
    ...
    void assertIsActive() const
    {
        if (m_stackSize == 0)
        {
            lua_pushstring(L, "Unable to continue registration");
            lua_error(L);
        }
    }
};
    ...
    public:
        ...
        */
        Class(char const* name, Namespace& parent, void const* const staticKey) : ClassBase(parent)
        {
            ...
            {
                lua_pushstring(L, "Base class is not registered");
                lua_error(L);
                // Код після lua_error() не виконується, але для компілятора:
                return; 
            }
        ...
        */
        ...
        Class<T>& addFunction(char const* name, ReturnType (T::*mf)(Params...))
        {
            ...
            if (name == GC)
            {
                // Замість throw std::logic_error
                lua_pushstring(L, "__gc metamethod registration is forbidden");
                lua_error(L);
                return *this; // Для компілятора (ніколи не виконається)
            }
