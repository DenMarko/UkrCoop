#ifndef _INCLUDE_SOURCEMOD_CSTRING_PROPER_H_
#define _INCLUDE_SOURCEMOD_CSTRING_PROPER_H_
...
public:
	...
	String operator +(const char *other) const
	{
		...
		size_t len = size() + other_len;
		char *buf = (char*)malloc(len + 1);
		memcpy(buf, c_str(), size());
		memcpy(buf + size(), other, other_len);
		buf[len] = '\0';
	...
	String operator +(const String& other) const
	{
		...
		char *buf = (char*)malloc(len + 1);
		memcpy(buf, c_str(), size());
		memcpy(buf + size(), other.c_str(), other.size());
		buf[len] = '\0';
