...
char *_get_name (GeoIP* gi, unsigned long ipnum) {
	...
	if (gi->cache == NULL) {
		...
		buf[MAX_ORG_RECORD_LENGTH - 1] = '\0'; // Ensure null termination
		len = strlen(buf) + 1;
		org_buf = malloc(len);
		if (org_buf != NULL) {
			strcpy(org_buf, buf);  // Safe - we allocated exactly the right size
		}
	} else {
		...
		org_buf = malloc(len);
		if (org_buf != NULL) {
			strcpy(org_buf, buf_pointer);  // Safe - we allocated exactly the right size
		}
	}
	return org_buf;
}
