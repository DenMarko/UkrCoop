...
WitchEvilAttack::WitchEvilAttack(ITerrorPlayer *pAttacker, bool bSoundPlay) :
    m_soundPlay(bSoundPlay), 
...
EventDesiredResult<IWitch> WitchEvilAttack::OnContact(IWitch *me, CBaseEntity *other, CGameTrace *result)
{
    if(!other)
    {
        return TryToSustain();
...
ActionResult<IWitch> WitchEvilAttack::OnStart(IWitch *me, Action<IWitch> *priorAction)
{
...
ActionResult<IWitch> WitchEvilAttack::Update(IWitch *me, float interval)
{
    ...
    if(!pVictim->IsAlive())
    {
        ...
        {
            CSearchBestPlayer search(me->GetAbsOrigin());
            ForEachSurvivor(search);
            if(!search.GetPlayer())
            {
                if(me->IsOnFire())
                {
    ...
    && !pBot->IsRangeGreaterThan((CBaseEntity*)pVictim, 30.f))
    {
        ...
        {
            CSearchBestPlayer serch(me->GetAbsOrigin());
            ForEachSurvivor(serch);
            if(!serch.GetPlayer())
            {
    ...
    {
        float flAttackRang = 1.2 * m_AttackRange.GetFloat();
...
EventDesiredResult<IWitch> WitchEvilAttack::OnMoveToFailure(IWitch *me, const Path *path, MoveToFailureType reason)
{
    if(reason)
    {
    ...
    CSearchBestPlayer serch(me->WorldSpaceCenter());
    ForEachSurvivor(serch);
    if(serch.GetPlayer())
    {
        SetVictim(serch.GetPlayer());
...
void WitchEvilAttack::SetVictim(ITerrorPlayer *victim)
{
    if(victim)
    {
...
ITerrorPlayer *WitchEvilAttack::GetVictim()
{
...
EventDesiredResult<IWitch> WitchEvilAttack::OnInjured(IWitch *me, const CTakeDamageInfo &info)
{
    return TryToSustain();
}

EventDesiredResult<IWitch> WitchEvilAttack::OnShoved(IWitch *me, CBaseEntity *pusher)
{
    return TryToSustain();
}

EventDesiredResult<IWitch> WitchEvilAttack::OnMoveToSuccess(IWitch *me, const Path *path)
{
    auto pBody = me->GetBodyInterface();
...
EventDesiredResult<IWitch> WitchEvilAttack::OnCommandAttack(IWitch *me, CBaseEntity *victim)
{
    return TryToSustain(RESULT_TRY);
}

void WitchEvilAttack::OnEnd(IWitch *me, Action<IWitch> *nextAction)
{
    me->RemoveGesture(ACT_TERROR_ATTACK);
    me->SetFlexWeight("bite", 0.0);
}

EventDesiredResult<IWitch> WitchEvilAttack::OnAnimationEvent(IWitch *me, animevent_t *pEvent)
{
    if(pEvent->event == 45)
    {
